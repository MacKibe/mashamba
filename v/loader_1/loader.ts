...
import {key, quiz, discriminant, dirty} from "../../../mlima/v/member/quiz.js";
...
export class loader extends quiz<Iloader>{
  ...
  public async execute(input: Iloader): Promise<Array<Error>>{
    //
    ...
    //
    //Alert the result
    if (result === "ok") return []; else return [new Error(result)];
  }
  ...
  public report_targeted_error(id:key<Iloader>, error:Error):undefined{
    //
